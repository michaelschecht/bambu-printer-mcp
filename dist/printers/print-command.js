...
export async function publishPrintCommand(printer, payload, timeoutMs = 10000) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onMessage = (_topic, raw) => {
            let report;
            try {
                report = JSON.parse(raw.toString()).print;
            }
            catch {
                return;
            }
            if (!report || report.command !== payload.print.command || String(report.sequence_id) !== sequence)
                return;
